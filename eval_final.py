...
checkpoint = '../a-PyTorch-Tutorial-to-Image-Captioning/BEST_checkpoint_coco_5_cap_per_img_5_min_word_freq.pth.tar'  # model checkpoint
...
def evaluate(beam_size):
    ...
    for i, (image, caps, caplens, allcaps) in enumerate(
        ...
        if len(complete_seqs_scores) == 0:
            print('INCOMPLETE SEQ SCORE: ', str(i))
            continue
    ...
    #hypo = list(list(sentence)) list(list(words))
    #ref = list(list(list(sentence)))
    return hypo, ref
    # do_pos_counting(ref,hypo)
    # print("DONE POS COUNTING, STARTING SCORING")
    # score = []
    # method = []
...
def do_pos_counting(gts, res):
    pos_count_map = defaultdict(int)
    fd = nltk.FreqDist()
    for i in range(len(res)):
        ...
        assert(len(ref) >= 1)
        tokenized = nltk.word_tokenize(hypo[0])
        token_words = [rev_word_map[int(w)] for w in tokenized]
        tagged = nltk.pos_tag(token_words)
        for item in tagged:
            fd[item[1]]+=1
            pos_count_map[item[1]]+=1
    fd.most_common()
